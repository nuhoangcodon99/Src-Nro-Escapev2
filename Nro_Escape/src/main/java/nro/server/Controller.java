...
public class Controller {
        ...
        try {
            ...
            switch (cmd) {
                ...
                case -101:
                    ...
                    // PreparedStatement ps = con.prepareStatement("SELECT id FROM account ORDER BY
                    // id DESC LIMIT 1");
                        ...
                        if (player.MaBaoVe != 0) {
                            if (mabaove == player.getSession().MaBaoVe) {
                                if (!player.isUseMaBaoVe) {
                                    NpcService.gI().createMenuConMeo(player, 25100305, -1,
                                            "Tài khoản không được bảo vệ\nBạn muốn bật chức năng bảo vệ tài khoản?",
                                            "Đồng ý", "Từ chối");
                                    break;
                                } else {
                                    NpcService.gI().createMenuConMeo(player, 25100304, -1,
                                            "Tài khoản đang được bảo vệ\bBạn có muốn tắt bảo vệ không?", "Đồng ý",
                                            "Từ chối");
                                    break;
                                }
                            } else {
                            ...
                            NpcService.gI().createMenuConMeo(player, 25100303, -1,
                ...
                case -105: // Nhẫn thời không
                    if (player.type == 0 && player.maxTime == 30) {
                        ...
                        if (MapService.gI().isMapKhiGas(player.zone.map.mapId)) {
                            ChangeMapService.gI().changeMap(player, player.location.goMap, -1, player.location.goX,
                                    player.location.goY);
                        }
                    ...
                    if (player != null) {
                        switch (player.iDMark.getTypeChangeMap()) {
                            ...
                            default:
                                ...
                                if (boss.zone != null) {
                                    ChangeMapService.gI().changeMap(player, boss.zone.map.mapId, boss.zone.zoneId,
                                            boss.location.x, boss.location.y);
                                    break;
                                } else {
                                    Service.getInstance().sendThongBao(player,
                                            "Boss này chưa xuất hiện hoặc đang chết");
    ...
    // try {
    // Connection con = DBService.gI().getConnectionForGame();
    // PreparedStatement ps = con.prepareStatement("INSERT INTO account (username,
    // password) VALUES (?, ?)");
    ...
    public void sendInfo(Session session) {
        ...
        if (yajiro == null) {
            try {
                Boss_Yanjiro dt = new Boss_Yanjiro(Util.createIdDuongTank((int) player.id), BossData.YANJIRO,
                        player.zone, -1, -1, player.id);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
        ...
        if (mrpopo == null) {
            try {
                Boss_MrPôPô dt = new Boss_MrPôPô(Util.createIdDuongTank((int) player.id), BossData.MR_POPO, player.zone,
                        -1, -1, player.id);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        ...
        if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
            ...
            if (player.gender == 0) {
                npcSay += "Mình là Ngoccoder sẽ đồng hành cũng bạn trên thế giới này\n";
            ...
            // if (player.isAdmin()) {
            // Service.gI().ServerMessageVip("Thông báo này chỉ là thông báo thử nghiệm của
            // Client Version 2.3.7");
            // }
        }
