...
public class PetService {
    ...
    public void changeSuperPet(Player player, int gender, int typePet) { // typePet = 0 : Goku, typePet = 1 : Vegeta, typePet = 2 : Picolo
        byte limitPower = player.pet.nPoint.limitPower;
        if (player.fusion.typeFusion != ConstPlayer.NON_FUSION) {
            player.pet.unFusion();
        }
    ...
    private void createNewPet(Player player, boolean isMabu, boolean isGokuSSJ4, boolean isVegetaSSJ4, boolean isSuperPicolo, byte... gender) {
        int[] data = isMabu ? getDataPetMabu() : isGokuSSJ4 ? getDataPetSuper() : isVegetaSSJ4 ? getDataPetSuper() : isSuperPicolo ? getDataPetSuper() : getDataPetNormal();
        Pet pet = new Pet(player);
        pet.name = "$" + (isMabu ? "Mabư" : isGokuSSJ4 ? "Goku SSJ4" : isVegetaSSJ4 ? "Vegeta SSJ4" : isSuperPicolo ? "Super Picolo" : "Đệ tử");
        pet.gender = (gender != null && gender.length != 0) ? gender[0] : (byte) Util.nextInt(0, 2);
        pet.id = -player.id;
        pet.nPoint.power = isMabu ? 1500000 : isGokuSSJ4 ? 1500000 : isVegetaSSJ4 ? 1500000 : isSuperPicolo ? 1500000 : 2000;
        pet.isMabu = isMabu;
