...
public class Service {
        ...
        try {
            ...
            for (int i = 0; i < BossManager.BOSSES_IN_GAME.size(); i++) {
                ...
                if (boss.zone != null) {
                    ...
                    msg.writer().writeUTF("Map: " + boss.zone.map.mapId + ",Zone: " + boss.zone.zoneId);
                } else {
                    msg.writer().writeUTF("Boss: " + boss.name);
    ...
    public void chat(Player player, String text) {
        if (player.getSession() != null && player.isAdmin()) {
            ...
            if (text.equals("buffop")) {
                Input.gI().createFormSenditem1(player);
                return;
            }
            if (text.equals("tansat")) {
            ...
            if (text.equals("next_game_csmm")) {
                Service.gI().sendThongBao(player,
                        "Kết quả con số may mắn tiếp theo là: " + MiniGame.gI().MiniGame_S1.result_next);
                return;
            }
            if (text.equals("a")) {
                showListBoss(player);
            ...
            if (text.equals("adm")) {
                ...
                NpcService.gI().createMenuConMeo(player, ConstNpc.MENU_ADMIN, -1,
                        "|7|Admin Escape\n"
                                + "|7|YouTube Ngoccoder\n"
                                + "|5|Số lượng người chơi hiện tại: "
                                + Util.mumberToLouis(Client.gI().getPlayers().size()) + ".\n"
                                + "|5|Số lượng thread hiện tại: " + Thread.activeCount() + ".\n"
                                + "|5|Số lượng Session hiện tại: " + Client.gI().sessions.size(),
